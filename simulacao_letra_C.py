...
def plot_results(sol, omega_y, error_percent, T_hold=1.0, save_filename=None, is_limit_case=False):
    ...
    title_text = f'Item C: Resposta para $\omega_y$ = {omega_y:.2f} rad/s (Erro = {error_percent:.2f}%)'
    if is_limit_case:
        title_text = f'Item C: Resposta na Frequência Limite Teórica ($\omega_y$ = {omega_y:.2f} rad/s, Erro = {error_percent:.2f}%)'
...
T_hold_duration = 1.0
print("--- Item C: Iniciando busca por frequências... ---")
omegas = np.linspace(0.01, 1.5, 300)
...
for label, res_data in results.items():
    ...
    if freq > 0:
        ...
        filename_to_save = f"item_C_grafico_{filename_suffix}"
...
filename_limite = "item_C_grafico_limite_teorico"
